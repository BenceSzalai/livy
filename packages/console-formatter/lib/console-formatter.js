...
export class ConsoleFormatter extends AnsiLineFormatter {
     ...
     */
    flowLevel;
    constructor({ flowLevel = 2, ...options } = {}) {
        super(options);
     ...
     */
    formatContext(context, ignoreEmpty) {
     ...
     */
    cloneAndReplace(container, value, replacement, cache = new Map()) {
        if (container === value) {
            return replacement;
        }
        ...
        if (Array.isArray(container)) {
            const result = container.map(entry => this.cloneAndReplace(entry, value, replacement, cache));
    ...
    formatData(object, ignoreEmpty) {
        if (isEmpty(object)) {
            ...
            sanitizedObject = this.cloneAndReplace(sanitizedObject, undefined, undefinedReplacement);
            sanitizedObject = this.cloneAndReplace(sanitizedObject, null, nullReplacement);
            ...
            serializedYaml = serializedYaml
                .replaceAll(new RegExp(undefinedReplacement, 'g'), chalk.gray('undefined'))
