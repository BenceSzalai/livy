...
export class MemoryUsageProcessor {
     ...
     */
    convertToHumanReadable(raw) {
        const units = ['Bytes', 'KB', 'MB', 'GB', 'TB'];
        if (typeof raw !== 'number' || !Number.isFinite(raw) || raw < 0) {
            throw new Error(`Unexpected argument: Finite, non-negative number expected, received ${JSON.stringify(raw)}`);
        }
        let reduced = raw;
        ...
        for (let i = 0; i < units.length; i++) {
            ...
            if (reduced < 1024 || isLargestUnit) {
                ...
                let roundedValue = reduced < 100 ? this.round(reduced, 1) : this.round(reduced);
        ...
        throw new Error('Unexpected error when converting memory usage to human readable format');
        /* c8 ignore stop */
    }
    /**
     * @inheritdoc
